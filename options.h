...
const boost::program_options::options_description & getClientOptionsDescription() {
	...
	if (!initialized) {
		clientOptionsDescription.add_options()
			...
			("gui-address,d", value<std::string>()->required(), "The address of the GUI server")
			("player-name,n", value<std::string>()->required(), "The name identifying you in the game")
			("port,p", value<port_t>()->required(), "The port on which the client will be listening")
...
const boost::program_options::options_description & getServerOptionsDescription() {
	using namespace boost::program_options;
	...
	if (!initialized) {
		serverOptionsDescription.add_options()
			("help,h", "Display this help message")
			("bomb-timer,b", value<uint16_t>()->required(), "The number of turns after which a bomb explodes")
			("players-count,c", value<uint8_t>()->required(), "The number of players")
			("turn-duration,t", value<uint64_t>()->required(), "The duration of one turn in milliseconds")
			("explosion-radius,e", value<uint16_t>()->required(), "The radius of explosions")
			("initial-blocks,k", value<uint16_t>()->required(), "The initial number of blocks on the board")
...
boost::program_options::variables_map parseOptions(int argc, char * * argv, const boost::program_options::options_description & optionsDescription) {
	using namespace boost::program_options;
